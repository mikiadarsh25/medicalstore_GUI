...
connector = sqlite3.connect('MEDICALSTORE.db')
cursor = connector.cursor()

connector.execute(
"CREATE TABLE IF NOT EXISTS MEDICAL_STORE (PATIENT_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
"NAME TEXT, EMAIL TEXT, PHONE_NO TEXT, GENDER TEXT, DATE TEXT, MEDICINE TEXT)"
)

# Creating the functions
def reset_fields():
    global name_strvar, email_strvar, contact_strvar, gender_strvar, dop, medicine_strvar

    for i in ['name_strvar', 'email_strvar', 'contact_strvar', 'gender_strvar', 'medicine_strvar']:
        exec(f"{i}.set('')")
...
def display_records():
    ...
    curr = connector.execute('SELECT * FROM MEDICAL_STORE')
    data = curr.fetchall()
...
def add_record():
    ...
    if not name or not email or not contact or not gender or not date or not medicine:
        mb.showerror('Error!', "Please fill all the missing fields!!")
    else:
        try:
            connector.execute(
            'INSERT INTO MEDICAL_STORE (NAME, EMAIL, PHONE_NO, GENDER, DATE, MEDICINE) VALUES (?,?,?,?,?,?)', (name, email, contact, gender, date, medicine)
            )
            connector.commit()
...
def remove_record():
    if not tree.selection():
        ...
        connector.execute('DELETE FROM MEDICAL_STORE WHERE PATIENT_ID=%d' % selection[0])
        connector.commit()
...
def view_record():
    ...
    dop.set_date(date); medicine_strvar.set(selection[6])



# Initializing the GUI window
main = Tk()
main.title('Avni Medical Store')
...
Label(main, text="Avni Medical Store", font=headlabelfont, bg='SpringGreen').pack(side=TOP, fill=X)
...
Label(left_frame, text="Name", font=labelfont, bg=lf_bg).place(relx=0.1, rely=0.05)
Label(left_frame, text="Contact Number", font=labelfont, bg=lf_bg).place(relx=0.1, rely=0.18)
Label(left_frame, text="Email Address", font=labelfont, bg=lf_bg).place(relx=0.1, rely=0.31)
Label(left_frame, text="Gender", font=labelfont, bg=lf_bg).place(relx=0.1, rely=0.44)
Label(left_frame, text="Date", font=labelfont, bg=lf_bg).place(relx=0.1, rely=0.57)
Label(left_frame, text="Medicine", font=labelfont, bg=lf_bg).place(relx=0.1, rely=0.7)
...
Entry(left_frame, width=19, textvariable=medicine_strvar, font=entryfont).place(x=20, rely=0.75)
...
tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                    columns=('Patient ID', "Name", "Email Address", "Contact Number", "Gender", "Date", "Medicine"))
